...
exports.addMedication = async (req, res) => {
    try {
      ...
      const medication = new Medication({
        ...
        frequency,
        times_a_day: times.length || frequency,
        times: times, // Storing notification times
        selectedDays,
        duration,
        startDate,
        endDate,
        isActive,
        notificationIds // Adding notification IDs array
      });
...
exports.getWeeklyMedications = async (req, res) => {
  try {
    ...
    medications.forEach(med => {
      // Get days where medication should be taken
      const activeDays = Object.entries(med.selectedDays || {})
        .filter(([day, isActive]) => isActive)
        .map(([day]) => day);
      
      // If no specific days are selected, use all days (daily medication)
      const daysToUse = activeDays.length > 0 ? activeDays : 
      ...
      // Add the medication to each day it should be taken
      daysToUse.forEach(day => {
        const fullDayName = dayMapping[day];
        if (!groupedMedications[fullDayName]) {
          groupedMedications[fullDayName] = [];
        }
        
        // Add the medication multiple times based on frequency/times
        const timesPerDay = med.times || 
                           (med.frequency ? Array(med.frequency).fill({}) : [{}]);
        
        timesPerDay.forEach((time, index) => {
          groupedMedications[fullDayName].push({
            ...med,
            doseNumber: index + 1,
            time: time
          });
    ...
    medications.forEach(med => {
      ...
      for (let date = new Date(start); date <= end; date.setDate(date.getDate() + 1)) {
        ...
        if (med.selectedDays[dayOfWeek]) {
          ...
          // Include all times/doses for this medication
          const timesPerDay = med.times || 
                             (med.frequency ? Array(med.frequency).fill({}) : [{}]);
          
          timesPerDay.forEach((time, index) => {
            groupedMedications[dateStr].push({
              ...med,
              doseNumber: index + 1,
              time: time
            });
...
// Add method to cancel notifications when medication is deleted or deactivated
