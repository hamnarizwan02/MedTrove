...
export default function Cart({ navigation }) {
  ...
  const calculateTotal = () => {
    const itemTotal = cartItems.Medicine?.reduce((total, medicine, index) => {
      let priceString = prices[medicine]; // Fetch price from prices object
  
      if (!priceString) {
        priceString = "100"; // Default to "100" if undefined
      }
  
      const price = typeof priceString === "string" ? parseFloat(priceString.replace(/[^0-9.-]+/g, "")) : parseFloat(priceString);
  
      const quantity = cartItems.Quantity[index] || 1; // Ensure quantity is valid
