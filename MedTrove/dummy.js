...
// Import the pharmacy data and functions from the separate file
...
const Pharmacy = ({ navigation }) => {
  ...
  const fetchNearbyPharmacies = async (latitude, longitude) => {
    try {
      ...
      if (data.status === 'OK') {
        ...
        // Create mock data if the API fails using the imported function
        const mockData = generateMockPharmacies(latitude, longitude, radiusKm);
      ...
      // Generate mock data on error using the imported function
      const mockData = generateMockPharmacies(latitude, longitude, radiusKm);
      setPharmacies(mockData);
      setLoading(false);
    }
  ...
  const selectPharmacy = (pharmacy) => {
    ...
    if (!searchLocation) {
      Alert.alert('Select a location first', 'Please search and select a location before selecting a pharmacy.');
      return;
    }
